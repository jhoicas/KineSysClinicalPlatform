...
import { AnthropometryModule } from './components/AnthropometryModule';
import { BodyCompositionModule } from './components/BodyCompositionModule';
import { NutritionPlanningModule } from './components/NutritionPlanningModule';
...
export default function App() {
  ...
  const [anthropometryData, setAnthropometryData] = useState<Record<string, AnthropometryAssessment>>(INITIAL_ANTHROPOMETRY);
  const [bodyCompositionData, setBodyCompositionData] = useState<Record<string, BodyCompositionBIA>>(INITIAL_BODY_COMPOSITION);
  const [nutritionPlanData, setNutritionPlanData] = useState<Record<string, NutritionPlan>>(INITIAL_NUTRITION_PLANS);
  ...
  const activeAnthropometry = anthropometryData[activePatient.id] || INITIAL_ANTHROPOMETRY['p-sol'];
  const activeBodyComposition = bodyCompositionData[activePatient.id] || INITIAL_BODY_COMPOSITION['p-sol'];
  const activeNutritionPlan = nutritionPlanData[activePatient.id] || INITIAL_NUTRITION_PLANS['p-sol'];
  ...
  const handleUpdateAnthropometry = (updated: AnthropometryAssessment) => {
    setAnthropometryData((prev) => ({ ...prev, [activePatient.id]: updated }));
  };

  const handleUpdateBodyComposition = (updated: BodyCompositionBIA) => {
    setBodyCompositionData((prev) => ({ ...prev, [activePatient.id]: updated }));
  };

  const handleUpdateNutritionPlan = (updated: NutritionPlan) => {
    setNutritionPlanData((prev) => ({ ...prev, [activePatient.id]: updated }));
  };
      ...
      <div className="flex flex-col flex-1 h-full min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
          ...
          {currentTab === 'anthropometry' && (
            <AnthropometryModule
              patient={activePatient}
              assessment={activeAnthropometry}
              onSave={handleUpdateAnthropometry}
            />
          )}
          ...
          {currentTab === 'bodycomposition' && (
            <BodyCompositionModule
              patient={activePatient}
              data={activeBodyComposition}
              onSave={handleUpdateBodyComposition}
            />
          )}
          ...
          {currentTab === 'nutritionplan' && (
            <NutritionPlanningModule
              ...
              onSave={handleUpdateNutritionPlan}
            />
          )}
