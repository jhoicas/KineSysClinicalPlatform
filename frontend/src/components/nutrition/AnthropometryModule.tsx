...
export const AnthropometryModule: React.FC<AnthropometryModuleProps> = ({
  ...
  const fatStatus: 'Bajo' | 'Rango saludable' | 'Sobrepeso' | 'Elevado' = (() => {
    ...
    if (patient.gender === 'F') {
      ...
      if (calculatedFatPct <= 35) return 'Sobrepeso';
      return 'Elevado';
    }
    if (calculatedFatPct < 6) return 'Bajo';
    if (calculatedFatPct <= 20) return 'Rango saludable';
    if (calculatedFatPct <= 25) return 'Sobrepeso';
    return 'Elevado';
  })();
